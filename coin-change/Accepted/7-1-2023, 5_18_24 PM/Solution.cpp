...
private:
    ...
    int calculateMinCoins(vector<int>& coins, int amount) {
        ...
        for (int coin : coins) {
            if (amount - coin >= 0) {
            // Calculate the minimum number of coins needed for the remaining 
            // amount after subtracting the current coin.
            // Increment the result by 1 to account for using the current coin
